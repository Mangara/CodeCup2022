...
/**
 * A fast, high-quality {@linkplain Random pseudorandom number generator} for
 * floating-point generation.
 * It has excellent speed,
 * but its state space (128 bits) is large enough for
 * mild parallelism only. It passes all tests we are aware of except for the
 * four
 * lower bits, which might fail linearity tests (and just those), so if
 * low linear complexity is not considered an issue (as it is usually the
 * case) it can be used to generate integer outputs, too; moreover, this
 * generator has a very mild Hamming-weight dependency making
 * <a href="http://prng.di.unimi.it/hwd.php">our test</a>
 * fail after 8 TB of output; we believe
 * this slight bias cannot affect any application. If you are concerned,
 * use {@link XoRoShiRo128StarStarRandom} or {@link XoShiRo256PlusRandom}.
 * More information can be found at our <a href="http://prng.di.unimi.it/">PRNG
 * page</a>.
 *
 * <p>
 * <strong>Warning</strong>: the constants used in this generator differ from
 * the ones used in the 2016 version.
 *
 * <p>
 * If you need a general PRNG, use {@link XoRoShiRo128StarStarRandom}. If you
 * can use more space,
 * you might try {@link XoShiRo256PlusRandom}.
 *
 * <p>
 * By using the supplied {@link #jump()} method it is possible to generate
 * non-overlapping long sequences
 * for parallel computations; {@link #longJump()} makes it possible to create
 * several
 * starting points, each providing several non-overlapping sequences, for
 * distributed computations. This class provides also a {@link #split()} method
 * to support recursive parallel computations, in the spirit of
 * {@link SplittableRandom}.
 *
 * <p>
 * <strong>Warning</strong>: before release 2.6.3, the {@link #split()} method
 * would not alter the state of the caller, and it would return instances
 * initialized in the same
 * way if called multiple times. This was a major mistake in the implementation
 * and it has been fixed,
 * but as a consequence the output of the caller after a call to
 * {@link #split()} is
 * now different, and the result of {@link #split()} is initialized in a
 * different way.
 *
 * <p>
...
public class XoRoShiRo128PlusRandom extends Random {
     ...
     * {@link XoRoShiRo128PlusRandom}
     * (seeded at startup with {@link System#nanoTime()}) and xoring it with
    ...
    /**
     * Returns a copy of this generator. The sequences produced by this
     * generator and by the returned generator will be identical.
     *
     * <p>
     * This method is particularly useful in conjunction with the
     * {@link #jump()} (or {@link #longJump()}) method: by calling repeatedly
     * {@link #jump() jump().copy()} over a generator it is possible to create
     * several generators producing non-overlapping sequences.
     *
     ...
     * can return only dyadic rationals of the form <var>k</var> /
     * 2<sup>&minus;52</sup>,
     ...
     * {@link #nextDouble()} is an additional least significant bit set in half
     * of the
    ...
    /**
     * The long-jump function for this generator. It is equivalent to
     * 2<sup>96</sup>
     * calls to {@link #nextLong()}; it can be used to generate 2<sup>32</sup>
     * starting points,
     * from each of which {@link #jump()} will generate 2<sup>32</sup>
     * non-overlapping
     ...
     * would not alter the state of the caller, and it would return instances
     * initialized in the same
     * way if called multiple times. This was a major mistake in the
     * implementation and it has been fixed,
     * but as a consequence the output of this instance after a call to this
     * method is
     * now different, and the returned instance is initialized in a different
     * way.
     *
     ...
     * whose output
     * will in turn be used to seed this generator. This approach makes
     * &ldquo;warmup&rdquo; unnecessary,
